...
void main() {
  ...
  testWidgets('Integration Test', (tester) async {
    ...
    await tester.enterText(find.byKey(Key('NameKey1')), 'Matheus');
    await tester.enterText(find.byKey(Key('EmailKey1')), 'matheus@email.com');
    ...
    expect(find.text('Matheus (Ferro)'), findsOneWidget);
    expect(find.byIcon(Icons.card_giftcard), findsOneWidget);

    expect(Provider.of<Clients>(providerKey.currentContext!, listen: false).clients.last.name, 'Matheus');
    expect(Provider.of<Clients>(providerKey.currentContext!, listen: false).clients.last.email, 'matheus@email.com');
  });
}